    ...
    <Element name="features">
      ...
      >
        ...
        >
          ...
          <Heading size={"xl"} zIndex={2} color={isDarkMode ? "#fff" : "#000"}  >Uifry Premium</Heading>

          {Data.map((item) => (
            <Box key={item.id} zIndex={2} position={"relative"}>
              <Box display={"flex"} gap={"1rem"} mt={"1rem"} >
                <Image src={item.img} alt="img2" />
                <Text fontWeight={"bold"} color={isDarkMode ? "wheat" : "#000"}> {item.heading}</Text>
              </Box>
              <Text zIndex={5} color={isDarkMode ? "#fff" : "#000"}> {item.text}</Text>
            </Box>
          ))}
        </Box>
      </Box>
